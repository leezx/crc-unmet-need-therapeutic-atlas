...
## PR #79 GSE225857 公开 GEO metadata sufficiency check

来源：同一对话 [CRC临床适应症地图](https://chatgpt.com/g/g-p-68c041b4df6881918a83a55e2dd7ac70/c/6a7a21a5-9474-83ea-895e-859b789fbe7c)。一轮审核；本轮审核结论由网页版 ChatGPT 给出，未向 GitHub 提交正式 review，审核记录保存在本对话及仓库归档中（修正 2026-08-24，PR #80 review：之前这里错误沿用了其他 PR 出现过的"GitHub 连接器 403"事件描述，但这一轮实际没有尝试过写回 GitHub，不能推断成同样失败——纯 event provenance 修正，不影响任何科学内容）。按 PR #78 round 3 reviewer 自己指定的下一步做的：不是继续做 access archaeology 也不是申请 DAC，而是真的把两个公开、无需申请的 metadata 文件下载下来看列结构。

初审 head `ed424f8`：真下载了 `GSM7058754_immune_meta.txt.gz`/`GSM7058755_non_immune_meta.txt.gz`（真实 SHA256 checksum、gzip 完整性验证，原始文件按已有 gitignore 规则放在 `modules/*/data_lock/raw/`，没有提交进 git）。列结构真的有 patient/site 分辨率：`patients` 列（immune 文件 7 个不同 ID，non-immune 文件 5 个）、`organs`/`samples` 列（immune 5 个 site code：CCL/CNL/LCL/LNL/PBL；non-immune 2 个：CCT/LCT）、`patients_organ` 组合列。site code 独立核实了原始论文（Wang et al., *Sci Adv* 2023, PMID 37327339）方法部分——CD45- nonimmune/CD45+ immune 细胞采自 primary colorectal cancer (CC)、adjacent normal colorectal mucosa (CN)、liver metastasis (LM)、adjacent normal liver (LN)、peripheral blood (PB)——行数和论文完全对得上（196,473 免疫 + 41,892 非免疫）。如实记录了一个没深挖的小矛盾：metadata 里 7 个 patient ID，论文说"6 个患者"。没有 per-cell treatment 列，但不需要——series 级 `treatment_annotation` 本来就统一。顺手把 `GSE225857` 从 `no_file_inventory_disposition.tsv` 转成真正的 `file_inventory.tsv`（2 行），跟仓库里全部 32 个 dataset 一直保持的"要么有 inventory 要么有 disposition，不会两个都有"约定一致，按 `GSE178318` 的先例转过去。网页版 ChatGPT 最终结论：**APPROVE**——scope 干净，只 materialize 了两个公开 metadata 文件的 checksum/inventory，没有偷跑 count-matrix 生物学分析；核实了外部 source（论文的 CC-vs-LM nonimmune comparison 本身就是 n=5 patients，所以 non-immune metadata 里恰好 5 个 ID 不构成新异常）；认可 file_inventory.tsv 转换（继续留着"没有 inventory"的 disposition 会变成假状态）。给了两条不阻断本 PR 的下一步 guardrail：(1) 7-vs-6 patient 矛盾必须继续保留为 unresolved，不能在下一轮静默把 immune file 当 7 个独立 patient；(2) 下一步应该比 PROJECT_STATUS 当时写得更窄——优先只下载 `GSM7058755_non_immune_counts.txt.gz`（86.2MB，不是两个都下），而且在重新造 EPCAM proxy 之前，先检查现有 metadata 是不是已经有作者提供的 tumor-cell/fibroblast/endothelial 标注（原论文确实把 nonimmune cells 分成了这三类）。

最终修正 head `d31a2ad`：直接查了已下载的 metadata（零额外成本，没有新下载），确认 non-immune 文件的 `cluster` 列真的带作者标注——11 个 `Tu01`-`Tu11` tumor cluster，跟论文"11 tumor cell clusters"完全对上；6 个 `E01`-`E06` endothelial cluster，跟论文"6 endothelial cell clusters"完全对上；6 个 `F01`-`F06` fibroblast cluster，比论文说的 8 个少，未核实。把 reviewer 的两条 guardrail 折进 `file_inventory.tsv`/`PROJECT_STATUS.md`/`data_lock/tgt_ceacam5.md`：下一步收窄为先下 non-immune count matrix、优先复用作者 tumor-cell 标注而不是重新造 EPCAM proxy、7-vs-6 patient 矛盾继续保留为 unresolved。不阻断本轮 review，PR 已 APPROVE，在合并前顺手补上（跟 PR #71 round 3、PR #74 round 2 的既有做法一致）。合并。

## PR #81 GSE225857 非免疫 tumor-cell prevalence screen（五个 target 全跑）

来源：同一对话 [CRC临床适应症地图](https://chatgpt.com/g/g-p-68c041b4df6881918a83a55e2dd7ac70/c/6a7a21a5-9474-83ea-895e-859b789fbe7c)。三轮审核；正式 review 同样因 GitHub 连接器 403 无法写回 PR，记录只保存在这里。按 PR #79 round 1 reviewer 自己指定的三步走顺序做的：cell-ID join → gene index 存在性 → 作者 tumor-cell 标注可直接复用，三个都确认过再算 prevalence。

初审 head `c5f250c`：真下载了 `GSM7058755_non_immune_counts.txt.gz`（86.2MB，SHA256+gzip 完整性验证），跑了五个 target 的完整 prevalence screen，限定在作者定义的 `Tu0N` tumor cluster 里（不是重新造的 EPCAM proxy）。核心计算本身可信；新增 `target_evidence.tsv` TE027-TE031、`evidence.tsv` EV035-EV039，全部 `evidence_directness=UNCALIBRATED_PROXY`。网页版 ChatGPT 结论：**REQUEST_CHANGES**——六个真实问题，不是文字挑剔：(1) 重新引入了 PR #77 已经关掉的跨 target ranking 语言("CEACAM5 broadest RNA_high footprint"、"ERBB2 flattest of the five")——per-gene RNA detection fraction 仍然不是校准过的跨基因量表；(2) 脚本宣称"built-in and fail-closed"的三个 pre-flight check，实际只做了 cell-ID join 和 gene index 两项，cluster 完整性/精确 Tu01-Tu11 集合/doublet 字段声明都只在文档里断言，代码里没真正校验；(3) 统计单位应该是 patient x site stratum，不是 cell，多处混用；(4) NECTIN4 的 RNA_no 数错了（说 3 个实际 4 个：s0107 LC/s0115 LC/s0920 CC/s0920 LC）；F3/NECTIN4 的 RNA_no 被错误归因于"两个 yield 最低的患者"，但 s0920 实际是这个 cohort 里 yield 最高的患者；(5) TE028-TE031 的"See also TE004"引用错了，四个非 CEACAM5 target 都被复制粘贴成 TE004，正确应该是 ERBB2→TE010、F3→TE015、NECTIN4→TE020、TACSTD2→TE025；(6) "source publication's validated tumor-cell clusters"措辞过头——原作者代码是先聚类再手动重命名（Tumorcell/Fibroblast/Endo），再对 Tumorcell 子集重新聚类命名 Tu01-Tu11，这是真实的作者标注但不是独立恶性验证；"all five patients received uniform preoperative chemotherapy/RT"也过头，registry 只支持"chemo and/or RT"，不能推出统一 modality。明确不要求重新做生物学分析，只要求修复措辞/校验/引用。

修正 head `697c9e6`（round 1）：六项全部修复——ranking 语言改成 assay-internal 的 stratum 计数并明确不构成跨 target 排名；新增真正的 `validate_metadata()`，对 cluster 非空/精确 Tu01-Tu11 集合/doublet 字段做真实 fail-closed 校验，加了 5 个针对性负向测试；单位统一改成 stratum；NECTIN4 计数和 s0920 yield 归因都改成如实描述；TE028-TE031 引用修正；"validated"降级为"author-defined"，"uniform"改成"chemotherapy and/or radiotherapy"。顺手把 `schemas/target_evidence.tsv` 里 TE004/TE010/TE015/TE020/TE025 notes、5 个 `question/tgt_*.md`、`PROJECT_STATUS.md`/`knowledge/README.md` 里残留的"GSE225857 remains blocked"现在时态说法全部扫过一遍，改成过去时态并指向 PR #81 的真实结果；`TARGET_EVIDENCE_PATTERN_COMPARISON.md`（已单独 review 并合并的 PR #77 报告）没有改正文，只加了带日期的更新说明。加完校验后五个 target 在真实 LOCAL 数据上重新跑了一遍，输出逐字节和之前完全一致。网页版 ChatGPT 复审：**REQUEST_CHANGES**——确认六项核心问题真正修对，核心计算没有需要重跑或重新解释的地方，但还有三个很小的收尾项：(1) GitHub PR description 还是旧版本，必须同步成当前措辞；(2) `validate_metadata()` 还缺 reviewer 上一轮明确要求的 patient set（`{s0107,s0115,s0813,s0920,s1231}`）和 organ set（`{CCT,LCT}`）exact-set fail-closed 检查——round 1 的测试 fixture 用了占位符 `pX`/`pY`，本身就没法测出这个缺口，代码对未知 organ 还会通过 `ORGAN_LABELS.get(organ, organ)` 静默继续算，这不是新要求，是上一轮就明确列在最低 fail-closed contract 里的；(3) stale blocker sweep 漏了一处——TE001（Module E 的行，不是新的 Module B 行）notes 里仍写"Module B, same target, currently BLOCKED"，因为措辞跟 grep 的 pattern 不完全一样没扫到。除此之外确认科学边界保持正确：ranking 已撤掉、GSE178318 与 GSE225857 不再声称定量复现、统计单位已正确、NECTIN4 4/10、s0920 正确识别为最高 yield、TE028-031 引用已修正、"uniform"已改、"validated"已降级、`UNCALIBRATED_PROXY` 没有因为第二 cohort 出现就升级。明确要求：不要再改方法/threshold/数字/重新设计 pipeline，只做这三项收尾。

最终修正 head `9f9ae51`（round 2）：`validate_metadata()` 补了 `EXPECTED_PATIENTS`/`EXPECTED_ORGANS` 两个 exact-set fail-closed 检查；测试 fixture 换成真实的 5 患者 + `CCT`/`LCT`，显式断言覆盖全部 expected 值，新增 2 个负向测试（未知 patient、未知 organ）；`TE001` 的 stale "currently BLOCKED" 改成过去时态指向 `TE027`；GitHub PR title/description 同步到当前措辞。加完两个新检查后五个 target 在真实 LOCAL 数据上又重新跑了一遍，输出逐字节和之前完全一致。网页版 ChatGPT 最终结论：**APPROVE**——三个 blocker 都真正关闭：`validate_metadata()` 现在对 patient set 和 organ set 做 exact-set fail-closed，未知值不会再静默通过；测试 fixture 已经是真实 cohort 并显式断言覆盖度，新增的两个负向测试确实测了 fail-closed path；`TE001` 修正到位；PR description 已同步，明确使用 patient x site strata、assay-internal descriptive pattern、chemotherapy and/or radiotherapy、非 quantitative replication 等措辞。核对 `697c9e6`→`9f9ae51` 只有 1 个新 commit，改动局限在 `target_evidence.tsv`、主分析脚本和测试文件，没有顺手改动核心计算或其他科学逻辑。CI 绿，PR open/draft/mergeable。科学结论边界保持正确：仍是 `UNCALIBRATED_PROXY`，第二个 treated CRLM cohort 增加的是独立观察证据，不是 surface-density proof、不是 CNV-confirmed malignancy，也没有重新引入跨 gene ranking。明确下一步建议：不要继续优化 GSE225857 这条 scRNA pipeline（CNV、immune matrix、更多 annotation 的边际决策价值已经明显下降）——下一阶段应该回到真正影响 ADC asset selection 的缺口。合并。

## PR #82 Module D 第一批真实肿瘤组织蛋白质证据（五个 target 全跑）

来源：同一对话 [CRC临床适应症地图](https://chatgpt.com/g/g-p-68c041b4df6881918a83a55e2dd7ac70/c/6a7a21a5-9474-83ea-895e-859b789fbe7c)。四轮审核均由网页版 ChatGPT 给出；最终 APPROVE 时尝试提交 GitHub formal review，但连接器返回 `403 Resource not accessible by integration`，因此 GitHub 上没有正式 review submission。各轮 verdict 现归档于仓库文档（修正 2026-08-25，PR #83 review：之前这里写"记录只保存在这里"并暗示每轮都尝试过写回 GitHub，但实际只有最终 APPROVE 时尝试过，且现在 verdict 也已同时归档进 PR #82 自己的 body——纯 event provenance 修正，不影响任何科学内容）。按 PR #81 round 3 reviewer 明确建议（停止继续投入 GSE225857，转回真正影响 ADC asset selection 的缺口）和用户明确选择，第一次激活 Module D。

初审 head `010a592`：把 Module D 的四个 `CORE_ACTIVE`/`CORE_SUPPORT` 候选数据集全部调查了一遍——本环境没有任何蛋白质组学 search engine 软件（MaxQuant/Proteome Discoverer/Spectronaut），任何原始质谱文件都是死路。真正能用的有两个：`PXD055821` 的一个很小（3.48MB）、已处理好的 DIA-NN gene-group intensity matrix（新脚本 `scripts/extract_pxd055821_protein_abundance.py`），和 `HPA_CRC_cancer_tissue` 的 `cancer_data.tsv.zip`（1.72MB）Pathology Atlas 表（新脚本 `scripts/extract_hpa_cancer_ihc.py`）。`PXD022613` 调查后实际 deprioritize（未 reclassify，867MB RAR 没有办法通过 HTTP 分段提取，且它自己发表的补充表里根本没有这五个 target）；`MCRC_liver_metastasis_PDO_2026` 的 14-marker mIHC panel 检查过，但只覆盖 `ERBB2`，没有作为主证据用。新增 `target_evidence.tsv` TE032-TE041、`evidence.tsv` EV040-EV049，全部 `evidence_directness=UNCALIBRATED_PROXY`。网页版 ChatGPT 结论：**REQUEST_CHANGES**——五个真实问题：(1) 重新引入了 PR #77 已经关掉的跨 target ranking 语言，还用"不是 calibrated ranking，但这是事实"这种 hedge 掩盖，但 hedge 本身不能把 ranking 变成非 ranking；(2) HPA cancer IHC 的证据性质写错了——文档里写成"whole-tumor-section IHC, not malignant-cell-specific"，但 reviewer 自己查了 HPA 官方 cancer methods 页面，发现实际是 cancer-cell-focused 的分类标注（基于 staining intensity + fraction of positive cancer cells）；(3) `PXD055821` 的 60 个 matrix column 被泛称为"samples"，没有明确 publication-supported 的 replicate unit——原论文确认这是这个项目三个 cohort 之一（Sydney cohort）的 60 specimens from 51 patients（总项目是 152 specimens/111 patients），因此后续全部改为 specimen-level read，明确不是 patient-level prevalence；(4) `file_inventory.tsv`（"pg_matrix.tsv 没下载"）和 `PROJECT_STATUS.md`（"已下载但还没用"）之间存在直接的 provenance 矛盾；(5) `summarize_detection()` 把字面字符串 `"0"` 也算成 detected，跟 claim 文案里"nonzero intensity"的说法不一致——虽然实际数据里没有一个值正好是 0，所以没有算错任何数值，但代码和 contract 之间确实不一致。

修正 head `b20de3a`（round 1）：先独立核实了两个外部事实再动手——自己去查了 HPA 官方 cancer methods 页面（确认是 cancer-cell-focused IHC，不是 whole-section），也去查了原始论文全文 PMC12335997（确认 60 specimens from 51 patients，Sydney cohort，属于 152 specimens/111 patients 的三个 cohort 之一）；reviewer 提到的"8 个患者贡献多个 specimen"这个具体数字在拿到的正文里没有找到确切依据，所以没有照搬。ranking 语言全部改成 target 各自独立描述；HPA 定性全文修正（`evidence_directness` 保持不变）；`PXD055821` 单位改成 specimens/patients；`pg_matrix.tsv` provenance 统一成"available from PRIDE but not downloaded/not used"；`summarize_detection()` 改成排除字面 "0"，加了负向测试；两个抽取脚本在真实 LOCAL 数据上重新跑过，输出逐字节一致（因为原本就没有值正好是 0）。顺手在改 ranking 语言的过程中自己查出并修正了一个 reviewer 没提到的错误：`question/tgt_tacstd2.md` 把 ERBB2 的"13 个 distinct HPA tissue"（实际最广）错误安在了 TACSTD2 头上（TACSTD2 实际是 9 个，第二广）。网页版 ChatGPT 复审：**REQUEST_CHANGES**——确认五项核心问题真正修对，核心数值没有需要重算的地方，但发现修复在 4 处不完整：每一处都是*不同的字段或文件*用不同措辞独立复述了同一个事实，round 1 的精确字符串匹配修复函数没有覆盖到——`schemas/evidence.tsv` 的 `EV045`-`EV049` `metadata` 字段（还是旧的 HPA 定性）、抽取脚本自己的 docstring（round 1 完全没碰这个文件）、`PXD055821/source_manifest.tsv`（初始提交时写的，round 1 批次没再碰过）、以及 `question/tgt_nectin4.md`/`question/tgt_erbb2.md` 里残留的 hedge 式跨 target 排名语言（"不是 calibrated ranking，但这是事实"和没被抓到的"mid-range"措辞——reviewer 原话："加一句'不 calibrated'不会把 ranking 变成非 ranking"）。

修正 head `7cb365e`（round 2）：四项全部修复——不再假设一次字符串替换就能覆盖所有地方，而是广泛 grep 扫描每一个独立复述同一事实的字段/文件；`EV045`-`EV049` metadata、脚本 docstring（现在直接引用 HPA methods 页面原文）、`source_manifest.tsv` 全部统一成 specimens/patients + cancer-cell-focused IHC 措辞；`question/tgt_nectin4.md` 完全重写为只陈述 NECTIN4 自己的数字（用已经写对的 `data_lock/tgt_nectin4.md` 作模板）；`question/tgt_erbb2.md` 的"mid-range"改成它实际的 `RNA_low` bucket 措辞。两个抽取脚本在真实 LOCAL 数据上对全部 5 个 target 重新跑过，检出数字和之前完全一致。PR 标题/正文也同步到修复后的内容。网页版 ChatGPT 结论：**REQUEST_CHANGES**——确认 round 2 的四处修复全部落地，核心数值没有变化，但还剩两个一行级别的 consistency/provenance 收尾：`DATA/registry/module_classification.tsv` 里 `HPA_CRC_cancer_tissue` 那行还在描述文件 inventory 建立之前的状态（因为这个 PR 已经删掉了 `no_file_inventory_disposition.tsv` 并建了真实的 `file_inventory.tsv`）；`HPA_CRC_cancer_tissue/source_manifest.tsv` 把 `cancer_data.tsv.zip` 称为"colorectal-scoped"，但这个文件本身其实是 HPA 的 pan-cancer Pathology Atlas 表，这次分析只是从里面选了 colorectal 那一行。
...
修正 head `d285c17`（round 1）：五项全部修复，没有重算任何数字——全文（脚本 docstring、analysis contract、data_lock、question、module README、PROJECT_STATUS.md、knowledge/README.md、target_evidence.tsv/evidence.tsv 的 claim+notes+metadata、file_inventory.tsv）改成只陈述原论文实际说的内容，明确留白"到底是真实低表达、还是这个 multiplex assay 灵敏度限制、还是两者都有"；"excluded from every downstream analysis"收窄成"excluded from downstream analyses of this multiplex protein-expression panel"（因为论文后面确实还分析了 ERBB2 amplification、单标 IHC、anti-ERBB2 治疗反应）；structured_value 改成 n_rows=136;nonzero_numeric=136;fraction_nonzero_numeric=1.0;...;assay_detection_threshold=UNKNOWN;source_marker_excluded=TRUE;exclusion_reason=very_low_expression，claim 文案从"detected in 136/136"改成"all 136 rows contain positive numeric values"；脚本的 summarize_detection() 改名成 summarize_values()；EXCLUDED_MARKERS 改成 {KRT7: "no expression", ERBB2: "very low expression levels"}，回归测试也改成检查 marker-specific 的具体原因；AO485 → A0485 全局改掉；顺手按照 PR #82 round 2 教训的规矩广泛扫了一遍，抓到一处漏改的（file_inventory.tsv 里 data_s3 那行的 note 还是没改的旧版本措辞）。网页版 ChatGPT 复审：**REQUEST_CHANGES**——确认核心的机器语义修复和其余四项都真正落地，但还剩两项：GitHub PR description 还是 round-0 的旧内容（正好是 round 1 刚判错并修掉的那些措辞——跟 PR #79/#81/#82 出现过的同一类问题）；"too low to trust"这个上一轮明确要求删除的 trust/reliability 推断在脚本 docstring 和 analysis contract 里还残留两处。

最终修正 head `ab37184`（round 2）：两项都修了——"too low to trust"以及更温和的变体"trusted by the source authors or not"全部改成精确的、原文支持的说法（"原作者因为测出来的表达量很低，把 ERBB2 排除出了这个 multiplex 面板的下游分析"）；PR description 同步到当前修复后的内容，加了 Round 1-2 的 Review history 小节，测试数量更新为 25 项。网页版 ChatGPT 最终结论：**APPROVE**——确认两项都修好，round 1 的其余修复（respectively 语义、A0485 拼写、PDO 与 tissue 分离、67 patients denominator、UNCALIBRATED_PROXY/EXPLORATORY_UNDERPOWERED/LOW 边界）都保持正确，机器可读层依然干净（没有偷偷把 detection/prevalence claim 塞回去），CI 绿，PR mergeable。非阻断说明：原论文自己内部 A0485/AO485 拼写不一致（Key Resources Table 用 A0485，Methods 正文用 AO485）——A0485（结构化表格里的版本）确认为 canonical，不需要改。Reviewer 收尾评价：这个 PR 留下了一个很好的 negative/ambiguous-evidence 先例——数据不会因为原作者排除就被扔掉，但机器层绝不能把"有非零数字"偷换成"检测阳性"，也不能替原作者猜测排除的原因。合并。
