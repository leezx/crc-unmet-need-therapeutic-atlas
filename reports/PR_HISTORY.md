...
## PR #81 — GSE225857 non-immune tumor-cell screen for all five targets

- URL: [#81](https://github.com/leezx/crc-unmet-need-therapeutic-atlas/pull/81)
- Initial reviewed head `c5f250c`: per PR #79 round-1 reviewer's own 3-step order (cell-ID join → gene-index presence → author tumor-cell annotation reusability, all verified first), downloaded `GSM7058755_non_immune_counts.txt.gz` (86.2 MB, SHA256+gzip-integrity-verified) and ran a real prevalence screen for all five `A_CLINICAL` targets, restricted to the metadata's author-defined `Tu0N` tumor clusters (not an EPCAM-only proxy). 5 new `target_evidence.tsv` rows (`TE027`-`TE031`), 5 new `evidence.tsv` rows (`EV035`-`EV039`), all `evidence_directness=UNCALIBRATED_PROXY`. Web ChatGPT result: REQUEST_CHANGES — the core computation was sound, but six real problems in the surrounding contract/documentation: (1) reintroduced PR #77's already-closed cross-target ranking language ("CEACAM5 broadest RNA_high footprint", "ERBB2 flattest of the five") — per-gene RNA detection fractions are still not a calibrated cross-gene scale; (2) the script claimed all three pre-flight checks were "built-in and fail-closed" when only cell-ID join and gene-index presence actually were — cluster completeness, the exact `Tu01`-`Tu11` set, and the doublet-field claim were asserted in prose but never verified in code; (3) "cell" vs. "stratum" unit errors — the actual statistical unit is a patient x site stratum, not an individual cell; (4) a real count error (`NECTIN4`'s `RNA_no` strata: 3 claimed, 4 actual) and a wrong causal claim (`F3`/`NECTIN4`'s `RNA_no` attributed to "the two lowest-yield patients," when `s0920` is actually the cohort's highest-yield patient by a wide margin); (5) `TE028`-`TE031`'s "See also TE004" first-cohort cross-reference was copy-pasted for all four non-CEACAM5 targets — correct IDs are `TE010` (ERBB2), `TE015` (F3), `TE020` (NECTIN4), `TE025` (TACSTD2); (6) "source publication's validated tumor-cell clusters" overclaimed independent malignancy validation (the original authors' own code clusters, then manually renames/re-clusters into `Tu01`-`Tu11` — real author-defined labeling, not independent validation), and "all five patients received uniform preoperative chemotherapy/RT" overclaimed a shared regimen (the registry only supports "chemotherapy and/or radiotherapy" per patient). No biological re-analysis requested — only wording/validation/cross-reference fixes.
- Round 1 head `697c9e6`: all six items fixed — ranking language replaced with assay-internal stratum counts and explicit no-ranking-license caveats; a real `validate_metadata()` added (cluster non-empty, exact `Tu01`-`Tu11` set, doublet fields, all fail-closed with 5 new negative-case tests); unit language corrected to "stratum" throughout; `NECTIN4`'s count and the `s0920` yield attribution corrected; `TE028`-`TE031` cross-references fixed; "validated" downgraded to "author-defined"; "uniform" changed to "chemotherapy and/or radiotherapy". Also swept stale present-tense "GSE225857 remains blocked on CNSA access" claims across `target_evidence.tsv` (`TE004`/`TE010`/`TE015`/`TE020`/`TE025`), all 5 `question/tgt_*.md` files, `PROJECT_STATUS.md`'s earlier narrative, and `knowledge/README.md`'s 2026-08-23 paragraph, converting them to past tense with a forward pointer to `TE027`; `TARGET_EVIDENCE_PATTERN_COMPARISON.md` (a separately-reviewed, already-merged PR #77 report) got a dated update note instead of a body rewrite. All five targets re-run after adding `validate_metadata()` — output byte-identical to before. Web ChatGPT result: REQUEST_CHANGES — confirmed all six items genuinely fixed, core computation needed no re-run or reinterpretation, but three small closeout items remained: (1) the GitHub PR description was still the round-0 text and needed syncing; (2) `validate_metadata()` was still missing the two other cohort invariants the canonical claim depends on — the exact patient set (`{s0107,s0115,s0813,s0920,s1231}`) and exact organ set (`{CCT,LCT}`), not a new requirement (already specified in the prior round's minimum fail-closed contract) — the round-1 test fixture used placeholder `pX`/`pY` IDs, so it would not have caught this gap, and an unrecognized organ code would silently fall through `ORGAN_LABELS.get(organ, organ)` uncaught; (3) the stale-blocker sweep missed `TE001` (a Module E row, not one of the new Module B rows), whose notes field still said "Module B, same target, currently BLOCKED" in present tense — different wording than the pattern grepped for in round 1. Explicitly no further method/threshold/number changes requested.
- Final reviewed head `9f9ae51` (round 2): added `EXPECTED_PATIENTS`/`EXPECTED_ORGANS` exact-set fail-closed checks to `validate_metadata()`; rewrote the test fixture to use the real 5-patient/2-organ cohort (asserted to cover every expected value) and added 2 new negative tests (unknown patient, unknown organ); fixed `TE001`'s stale note to point at `TE027` in past tense; synced the GitHub PR title/description to current wording. All five targets re-run again after the new checks — output byte-identical. Web ChatGPT final review: **APPROVE** — confirmed all three closeout items genuinely fixed: `validate_metadata()` now fail-closes on patient-set and organ-set mismatches; the test fixture is a real positive case with explicit coverage assertions and the 2 new negative tests exercise real fail-closed paths; `TE001` corrected; PR description synced. Confirmed the round-1→round-2 diff was scoped exactly to the three requested items, no incidental changes to core computation or other scientific logic. Reaffirmed the evidence boundary held throughout: still `UNCALIBRATED_PROXY`, the second treated CRLM cohort adds independent observational evidence, not surface-density proof or CNV-confirmed malignancy, and no cross-gene ranking was reintroduced. Explicit next-step recommendation: stop investing further in the `GSE225857` scRNA pipeline (CNV, immune-fraction matrix, more annotation) — marginal decision value has dropped; the next phase should return to gaps that actually affect ADC asset selection. Merged.
- Scope: real second-cohort biological computation (not provenance-only, unlike PR #78/#79) — a genuine second, independent, treated CRLM cohort for all five `A_CLINICAL` targets, obtained entirely through GEO's public route, no CNSA DAC application ever filed or needed. `evidence_directness` held at `UNCALIBRATED_PROXY` throughout three review rounds despite the temptation to read a stronger tumor-cell-identity method as license for a stronger evidence tier. Establishes (and, after review, actually enforces in code, not just prose) a reusable fail-closed pre-flight pattern for future dataset-specific Module B scripts: verify every claim the downstream evidence depends on (cell-ID join, gene presence, cluster completeness and exact label set, doublet-filtering state, patient set, organ set) before computing anything, with negative-case tests proving each check actually trips.

## PR #82 — Module D first pass: real tumor-tissue protein evidence for all five targets

- URL: [#82](https://github.com/leezx/crc-unmet-need-therapeutic-atlas/pull/82)
- Initial reviewed head `010a592`: per PR #81 round-3 reviewer's explicit recommendation (stop investing further in `GSE225857`; return to gaps affecting ADC asset selection) and explicit user direction, activated Module D for the first time. Investigated all four `CORE_ACTIVE`/`CORE_SUPPORT` candidate datasets; no proteomics search-engine software (MaxQuant, Proteome Discoverer, Spectronaut) is available in this environment, ruling out raw mass-spec reprocessing for any of them. Two were genuinely usable: `PXD055821`'s small (3.48 MB), already-processed DIA-NN gene-group intensity matrix (new `scripts/extract_pxd055821_protein_abundance.py`), and `HPA_CRC_cancer_tissue`'s `cancer_data.tsv.zip` (1.72 MB) Pathology Atlas table (new `scripts/extract_hpa_cancer_ihc.py`). `PXD022613` deprioritized (867MB RAR with no partial-extraction path over HTTP, target genes absent from its own supplementary table); `MCRC_liver_metastasis_PDO_2026`'s 14-marker mIHC panel checked but only covers `ERBB2`, not used as primary evidence. 10 new `target_evidence.tsv` rows (`TE032`-`TE041`), 10 new `evidence.tsv` rows (`EV040`-`EV049`), all `evidence_directness=UNCALIBRATED_PROXY`. Web ChatGPT result: REQUEST_CHANGES — five real problems: (1) reintroduced PR #77's already-closed cross-target ranking language across multiple files, hedged with "not calibrated, but the plain fact" qualifiers that don't actually remove the ranking; (2) HPA cancer IHC mischaracterized as "whole-tumor-section IHC, not malignant-cell-specific" — reviewer's own check of HPA's cancer-methods page showed it's actually a cancer-cell-focused categorical annotation (staining intensity + fraction of positive cancer cells); (3) `PXD055821`'s 60 matrix columns were called "samples" with no publication-supported replicate unit locked down — actually 60 specimens from 51 patients (the project's Sydney cohort, one of three cohorts in a 152-specimen/111-patient study); (4) a direct provenance contradiction between `file_inventory.tsv` ("pg_matrix.tsv not downloaded") and `PROJECT_STATUS.md` ("downloaded... not yet used"); (5) `summarize_detection()` counted a literal `"0"` string as detected, inconsistent with the claim text's "nonzero intensity" — no observed value happened to be exactly zero, so no computed value was wrong, but the contract/code mismatch was real.
- Round 1 head `b20de3a`: all five items fixed after independently verifying both external claims first — fetched HPA's own cancer-methods page (confirmed cancer-cell-focused IHC, not whole-section) and the publication's own full text, PMC12335997 (confirmed 60 specimens from 51 patients, Sydney cohort, out of 152 specimens/111 patients total); deliberately did not propagate the reviewer's unverifiable "8 patients contributed multiple specimens" sub-claim since it could not be confirmed in the fetched text. Ranking language rewritten to target-local statements throughout; HPA characterization corrected everywhere (`evidence_directness` kept unchanged); `PXD055821` specimen/patient terminology corrected; `pg_matrix.tsv` provenance unified to "available from PRIDE but not downloaded/not used"; `summarize_detection()` fixed to exclude literal zero values, with new regression tests; both extraction scripts re-run against real LOCAL data — byte-identical output (no observed value was exactly zero). Also self-caught and fixed an unrelated error found while sweeping ranking language: `question/tgt_tacstd2.md` had wrongly attributed `ERBB2`'s 13-tissue HPA footprint (the actual broadest) to `TACSTD2` (which has 9, second-broadest). Web ChatGPT result: REQUEST_CHANGES — confirmed all five items genuinely fixed and no computed value needed to change, but found the fixes incomplete in 4 places: each was a *different field or file* independently restating the same fact in wording the round-1 exact-string-match fix functions didn't match — `schemas/evidence.tsv`'s `EV045`-`EV049` `metadata` field (still old HPA characterization), the extraction script's own docstring (never touched in round 1), `PXD055821/source_manifest.tsv` (written in the initial commit, never revisited), and residual hedged ranking language in `question/tgt_nectin4.md` and `question/tgt_erbb2.md` (a "not calibrated, but the plain fact" hedge and an untouched "mid-range" framing — reviewer: "adding 'not calibrated' does not turn a ranking into a non-ranking").
- Round 2 head `7cb365e`: all four items fixed — swept every remaining field/file independently restating the corrected facts rather than assuming a single string-replace pass caught everything; `EV045`-`EV049` metadata, the script docstring (now quotes HPA's methods page verbatim), and `source_manifest.tsv` all synced to specimens/patients + cancer-cell-focused IHC terminology; `question/tgt_nectin4.md` fully rewritten to state only `NECTIN4`'s own numbers (using the already-correct `data_lock/tgt_nectin4.md` as the template); `question/tgt_erbb2.md`'s "mid-range" replaced with its actual `RNA_low`-bucket framing. Both extraction scripts re-run against real LOCAL data for all 5 targets — identical detection numbers to before. PR title/body also synced to the corrected content. Web ChatGPT result: REQUEST_CHANGES — confirmed all four round-2 fixes landed and no evidence value changed, but found two remaining one-line consistency/provenance items: `DATA/registry/module_classification.tsv`'s `HPA_CRC_cancer_tissue` row still described a pre-file-inventory state (stale since the PR deleted `no_file_inventory_disposition.tsv` and created a real `file_inventory.tsv`), and `HPA_CRC_cancer_tissue/source_manifest.tsv` called `cancer_data.tsv.zip` "colorectal-scoped" when it's actually HPA's pan-cancer Pathology Atlas table (colorectal rows selected for this analysis).
- Final reviewed head `b2e9985` (round 3): both one-line items fixed — `module_classification.tsv`'s reason field updated to reflect the materialized file inventory (tier/status unchanged: `CORE_SUPPORT`/`CANDIDATE`); `source_manifest.tsv` corrected to describe the file's real pan-cancer scope. No analysis code, evidence values, or ontology touched. Web ChatGPT final review: **APPROVE** — confirmed both closeout items genuinely fixed, no new evidence/ontology/classification/computation blocker found, CI green, PR mergeable. Reviewer attempted to write a formal GitHub review back via the connector but got `403 Resource not accessible by integration` (consistent with prior PRs); the APPROVE verdict itself was unambiguous in the conversation. Merged.
- Scope: Module D's first real protein-layer evidence for all five `A_CLINICAL` targets, built entirely from small already-processed files despite no proteomics reprocessing software being available. Extends the cross-target-ranking discipline established in PR #77 (RNA layer) and reinforced in PR #81 to the protein layer — per-gene MS detection fractions and per-antibody IHC categorical levels are not calibrated cross-target scales, so all findings are reported per-target on that target's own numbers only, even when two independent protein sources happen to point the same direction for one target. Demonstrates, across four review rounds (three fix cycles), that a reviewer-flagged fact correction must be swept into *every* field and file independently restating it (canonical schema rows, provenance manifests, script docstrings, PR metadata) — a single exact-string-match fix pass reliably misses fields phrased differently, and "grep broadly after each fix, don't assume one pass caught everything" is the durable lesson.

## PR #83 — Archive PR #82 review history

- URL: [#83](https://github.com/leezx/crc-unmet-need-therapeutic-atlas/pull/83)
- Initial reviewed head `dab7155`: documentation-only, archives PR #82's full 4-round review history into `CHATGPT_CRC_CLINICAL_INDICATION_MAP_FEEDBACK.md`/`PR_HISTORY.md`/`PROJECT_STATUS.md`. Web ChatGPT result: REQUEST_CHANGES — four historical-accuracy corrections to the archive text itself, none touching PR #82's substantive record: (1) `PXD055821`'s 60 columns had been archived as "60 independent patient(s)/samples" — the actual issue at the initial reviewed head was an unlocked-down "sample" unit, not a false patient-count claim; (2) the Chinese archive's "PXD022613 调查后降级处理" could misread as a classification-tier downgrade, when it was deprioritized in practice and never reclassified; (3) the 403 event-provenance note overstated that every round attempted a formal GitHub review write, when only the final APPROVE round did, and also predated PR #82's own body now carrying its round history too; (4) `PR_HISTORY.md`'s Scope line said "three review rounds" when PR #82 actually had four.
- Final reviewed head `0d24320` (round 1 fix): all four items fixed, no PR #82 substantive content changed. Web ChatGPT final review: **APPROVE** — confirmed all four corrections landed, CI green, PR mergeable. Merged.
- Scope: documentation-only, historical-accuracy corrections to the archive of PR #82's review, not to PR #82 itself.

## PR #84 — Module D second pass: `MCRC_liver_metastasis_PDO_2026` `ERBB2` mIHC, with a critical reliability caveat

- URL: [#84](https://github.com/leezx/crc-unmet-need-therapeutic-atlas/pull/84)
- Initial reviewed head `d0e8c22`: per explicit user direction (PR #82's Next-handoff item `3e(b)`), downloaded and checksummed `MCRC_liver_metastasis_PDO_2026`'s `Data S3.xlsx` (114.6 KB, Mendeley `hr94h42xdc.3`) — a real, already-processed multiplex-IHC (mIHC) table. Only `ERBB2` (of this repository's five `A_CLINICAL` targets) is among the panel's fourteen markers. New `scripts/extract_pdo_erbb2_mihc.py`. Before writing any claim, independently fetched the source publication's own full text (PMC13293968) — the same discipline PR #82 round 1 established — and found the paper's own methods text states, verbatim, "KRT7 and ERBB2 were excluded from analysis due to no or very low expression levels, respectively." This finding materially changed the value proposition of the chosen direction, so it was surfaced to the user before implementation; explicit user decision: incorporate the data, but carry the caveat prominently everywhere it appears. One new `target_evidence.tsv` row (`TE042`), backed by `EV050`, `evidence_directness=UNCALIBRATED_PROXY` but `evidence_level=EXPLORATORY_UNDERPOWERED`/`confidence=LOW` (a downgrade from this module's usual tier). Also fixed two stale provenance notes caught while doing this: `source_manifest.tsv`'s blanket "no download performed" (now scoped correctly since `Data S3.xlsx` is downloaded) and the module README's pre-existing (and, until this PR, false) claim that real `ERBB2` mIHC data had already been "downloaded." Web ChatGPT result: REQUEST_CHANGES — the direction and architecture were sound (`TE042`/`EV050` as a separate row, `evidence_directness` unchanged, PDO-vs-tissue distinction, publication-stated 67-patient denominator, `mcrc_liver_metastasis` mapping all accepted), but the initial implementation had overclaimed the source publication's finding into "reagent unreliable"/"assay noise" language the source never uses; a machine-readable-layer bug encoded `detected=136;fraction=1.0` in `structured_value` when a nonzero value is not evidence of biological detection (an agent reading only the machine field, not the prose caveat, could misread this as strong positive evidence); an `EXCLUDED_MARKERS` dict bug collapsed `KRT7`'s and `ERBB2`'s two different exclusion reasons into one shared string, erasing the source text's own "respectively"; an antibody catalog typo (`AO485` should be `A0485`, per the source publication's own Key Resources Table); "raw per-PDO values" mischaracterized `Data S3.xlsx` as unprocessed when it is already normalized/processed.
- Round 1 head `d285c17`: all five items fixed, no number recomputed — reframed throughout (script docstring, analysis contract, data_lock, question, module README, `PROJECT_STATUS.md`, `knowledge/README.md`, `target_evidence.tsv`/`evidence.tsv` claim+notes+metadata, `file_inventory.tsv`) to state only what the source publication actually says, explicitly leaving open whether the finding reflects true low expression, an assay-sensitivity limit, or both; narrowed "excluded from every downstream analysis" to "excluded from downstream analyses of this multiplex protein-expression panel" (the same publication analyzes `ERBB2` amplification, single-plex IHC, and anti-`ERBB2` treatment response elsewhere); `structured_value` changed to `n_rows=136;nonzero_numeric=136;fraction_nonzero_numeric=1.0;...;assay_detection_threshold=UNKNOWN;source_marker_excluded=TRUE;exclusion_reason=very_low_expression`, claim text changed from "detected in 136/136" to "all 136 rows contain positive numeric values"; `summarize_detection()` renamed to `summarize_values()`; `EXCLUDED_MARKERS` fixed to `{"KRT7": "no expression", "ERBB2": "very low expression levels"}` with marker-specific regression tests; `AO485`→`A0485` fixed everywhere; also caught and fixed one file a narrower sweep would have missed — `file_inventory.tsv`'s `data_s3` note still carried the pre-fix framing. Web ChatGPT result: REQUEST_CHANGES — confirmed the core machine-semantics fix and all four other items landed correctly, but found two remaining items: the GitHub PR description was still round-0 stale content (referencing the exact framing round 1 had just corrected — a recurring pattern this session, PR #79/#81/#82 all needed the same kind of sync), and two residual instances of "too low to trust" (in the script docstring and analysis contract) that still made an unsupported trust/reliability inference the source publication does not make.
- Final reviewed head `ab37184` (round 2): both items fixed — "too low to trust" and the softer variant "trusted by the source authors or not" replaced throughout with the precise, source-supported statement ("the source authors excluded `ERBB2` from downstream analyses of this multiplex panel because measured expression was very low"); PR description synced to current content with a Review history section and the correct 25-check test count. Web ChatGPT final review: **APPROVE** — confirmed both items fixed, all round-1 corrections (`respectively` semantics, `A0485` spelling, PDO-vs-tissue separation, 67-patient denominator, `UNCALIBRATED_PROXY`/`EXPLORATORY_UNDERPOWERED`/`LOW` tier) held, machine-readable layer stays clean (no detection/prevalence claim smuggled back in), CI green, PR mergeable. Non-blocking note (no action required): the source publication itself has an internal `A0485`/`AO485` inconsistency between its Key Resources Table and its Methods prose — `A0485` (the structured table) confirmed as canonical. Reviewer's own closing note: this PR leaves a durable negative/ambiguous-evidence precedent — data is not discarded just because the source authors excluded it, but the machine-readable layer must never quietly convert "a nonzero value exists" into "detected," and must never guess at the source authors' own reason for an exclusion. Merged.
- Scope: Module D's first source-excluded, ambiguous-signal evidence row — distinct from every other evidence source in this repository so far, none of which carried a source-publication-stated exclusion. Establishes a reusable pattern for handling exactly this situation: report the data (nothing is discarded just because the original authors set a marker aside), quote the source's own reason for exclusion verbatim rather than characterizing it, explicitly refuse to adjudicate biology-vs-assay-sensitivity ambiguity the source itself does not resolve, and — the sharpest lesson of this PR — never let a machine-readable `structured_value` field encode a stronger claim ("detected") than the prose actually supports, since a downstream aggregator reading only that field would not see the caveat at all.
