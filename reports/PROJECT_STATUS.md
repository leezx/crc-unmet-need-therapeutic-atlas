...
This is PR #70 (`pivot/adc-target-repurposing-atlas`), currently in its **fourth commit round** after web-ChatGPT review (`CRC临床适应症地图` conversation, `Biotech ideas` project) returned `REQUEST_CHANGES` on rounds 1, 2, and 3. See "Review history" below.
...
- **PR #70, round 4** — this round. All three round-3 items addressed: Module C's `persistence` axis retired and split into `longitudinal_persistence` / `refractory_or_treated_presence`; `B_PRECLINICAL_ADC` now requires a real preclinical ADC construct, not antibody-internalization alone; the Next-handoff wording corrected (see "Completed this pivot" item 9 above). Not yet re-submitted for review.
