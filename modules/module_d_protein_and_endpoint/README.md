...
- `PXD055821`: raw `.raw`/`.pdResult`/`.msf` files are not usable in this environment (no proteomics search-engine software available) — but the project's own small, already-processed DIA-NN gene-group matrix (a 60-sample sub-cohort of the full 152) gives real, gene-symbol-indexed protein abundance for all five targets.
- `HPA_CRC_cancer_tissue`: the classic HPA "Pathology Atlas" `cancer_data.tsv.zip` gives real colorectal-cancer-specific IHC staining-level data (High/Medium/Low/Not detected patient counts) for all five targets, distinct from `HPA_normal_tissue` (Module E only).
