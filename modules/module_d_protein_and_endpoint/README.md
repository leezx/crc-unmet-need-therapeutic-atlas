...
**`ERBB2`/`TACSTD2` MS-vs-IHC discrepancy investigation, 2026-08-25.** Per Next-handoff item `3e(a)`: `analysis_contracts/erbb2_tacstd2_ms_ihc_discrepancy.md` investigates why `PXD055821` MS (frequent nonzero detection) and `HPA_CRC_cancer_tissue` IHC (mostly `Low`/`Not detected`) disagree for these two targets. **Does not resolve it.** Two things checked: (1) cohort composition (`PXD055821` is liver-metastasis-specific, HPA's cohort is not documented as such) — independently fetched HPA's own pathology pages and found this is genuinely unanswerable from what HPA discloses (no primary-vs-metastatic breakdown, no antibody-clone attribution for the specific cancer-tissue image); (2) same-matrix relative-abundance percentile (new `scripts/analyze_pxd055821_abundance_percentile.py`) — `ERBB2`/`TACSTD2` sit at the 35th/45th percentile of median intensity among the matrix's 9,263 genes (modest abundance), versus `CEACAM5` at the 99.3rd percentile (the only target with concordant MS/IHC signal) — one concrete, checkable partial factor, not proof. No `target_evidence.tsv` field changes; TE033/TE036/TE038/TE041 notes cross-reference this file.
