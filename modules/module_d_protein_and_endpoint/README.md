...
**Second pass, 2026-08-25 (`ERBB2` only).** `MCRC_liver_metastasis_PDO_2026`'s `Data S3.xlsx` (114.6 KB) downloaded and checksummed — real per-PDO mIHC values for `ERBB2` (the only one of the five `A_CLINICAL` targets in this dataset's 14-marker panel). **Carries a critical caveat, not clean corroboration**: the source publication's own methods text states, verbatim, that `ERBB2` "was excluded from analysis due to ... no or very low expression levels" in this panel — the source authors' own QC judged this specific multiplex reagent unreliable. See `analysis_contracts/pdo_erbb2_mihc.md` and `data_lock/tgt_erbb2.md` for the full caveat, which must accompany every use of this evidence. One new `target_evidence.tsv` row (`TE042`, `evidence_level=EXPLORATORY_UNDERPOWERED`, `confidence=LOW`).
