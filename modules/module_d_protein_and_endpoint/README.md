...
**`PXD055821` protein-group-level confirmatory check, 2026-08-25.** Downloaded and checksummed the DIA-NN protein-group-level matrix (`pg_matrix.tsv`, same 60-specimen Sydney cohort) separate from the gene-group matrix `TE032`-`TE036` are built from — checked whether any of the five `A_CLINICAL` targets has isoform/proteoform splitting or protein-inference ambiguity the gene-group summary would collapse. **No difference found**: each target maps to exactly one single-UniProt-accession protein group, byte-identical values to the gene-group matrix. Confirmatory only — no `target_evidence.tsv` field changes. See `analysis_contracts/pxd055821_protein_abundance.md`.
