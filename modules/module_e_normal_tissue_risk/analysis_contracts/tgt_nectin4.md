...
- `NECTIN4` RNA and IHC are both **strongly skin-dominated**: skin RNA (64.1 HPA nTPM, 227.5 GTEx median TPM) is by far the highest of any compartment tracked (colon/rectum/GI compartments are all <2 nTPM), and the IHC layer directly corroborates this — **skin keratinocytes and epidermal cells both score Medium**, consistent with the independently well-documented clinical skin toxicity (rash, and rare severe cutaneous reactions) of enfortumab vedotin, the approved anti-`NECTIN4` ADC. As with `ERBB2`'s cardiac/pulmonary signal, this repository's own screen reproduces a known, real clinical liability directly from HPA data, without needing external literature to surface it.
- The IHC layer also shows squamous-epithelium involvement beyond skin (esophagus, oral mucosa, tonsil — all Medium, squamous epithelial cells), a coherent pattern across squamous/keratinized epithelium, not scattered across unrelated tissues.
- Colon/rectum RNA is very low (1.4 / 1.6 nTPM) with no IHC row scored High/Medium in GI tissue — `NECTIN4` shows essentially no GI-mucosal signal, a materially different pattern from `CEACAM5`.
