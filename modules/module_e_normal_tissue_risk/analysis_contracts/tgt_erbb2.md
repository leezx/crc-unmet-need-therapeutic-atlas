...
- Unlike `CEACAM5`, `ERBB2` RNA is **not GI-mucosa-dominated** — colon (63.8) and rectum (45.5) are mid-range, well below skin (84.8), esophagus (77.7), parathyroid gland (76.8) and fallopian tube (60.4). Expression is broad across many normal epithelial/glandular tissues, consistent with `ERBB2`'s known role as a widely-expressed receptor tyrosine kinase, not a GI-restricted antigen.
- The IHC layer shows a real protein-level signal not visible from RNA alone: **cardiomyocytes (Medium)** and **lung alveolar cells type I (Medium)**. Cardiac and pulmonary tissue are known clinically-monitored toxicity domains for approved anti-HER2 ADCs (trastuzumab deruxtecan, trastuzumab emtansine), but this run only reports what HPA's cell-type IHC panel shows for `ERBB2` protein expression in those two cell types — it does not establish, and is not evidence of, a causal on-target/off-tumor toxicity mechanism, and does not cite or rely on any external clinical-trial or label data to make that claim (corrected 2026-08-24, PR #76 round 1 review — an earlier version of this document overclaimed the IHC finding as "reproducing a known, real liability," which asserts a causal/clinical link this screen cannot support without a cited provenance object; that framing belongs in a future Module F/clinical-precedent pass, not here).
