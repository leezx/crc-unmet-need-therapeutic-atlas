...
- The IHC layer shows a real, clinically-recognized normal-tissue signal not visible from RNA alone: **cardiomyocytes (Medium)** and **lung alveolar cells type I (Medium)** — both consistent with the independently well-documented cardiac and pulmonary on-target/off-tumor toxicities of clinical anti-HER2 ADCs (trastuzumab deruxtecan, trastuzumab emtansine). This run did not need external literature to surface this signal; it fell directly out of the HPA IHC cell-type panel, a useful corroboration that this repository's screening pipeline reproduces a known, real liability rather than missing it.
