...
**Remaining four targets run 2026-08-24**: `tgt_erbb2` (`ERBB2`), `tgt_f3` (`F3`), `tgt_nectin4` (`NECTIN4`), `tgt_tacstd2` (`TACSTD2`) — the four other `ADC_TARGET_SEED_UNIVERSE.tsv` targets with documented ADC precedent, same corrected method as `CEACAM5` throughout (no round-1/round-2 correction needed this time; lessons already applied on the first pass). 15 new `target_evidence.tsv` rows (`TE007`-`TE009` `ERBB2`, `TE012`-`TE014` `F3`, `TE017`-`TE019` `NECTIN4`, `TE022`-`TE024` `TACSTD2`; RNA HPA + RNA GTEx + IHC per target), all `evidence_directness=UNCALIBRATED_PROXY`. Three real, notable findings this run independently reproduced without external literature: `ERBB2`'s IHC layer scores cardiomyocytes and lung alveolar cells Medium (matches clinical anti-HER2 ADCs' documented cardiac/pulmonary toxicity); `NECTIN4` and `TACSTD2` are both strongly skin-dominated in RNA and IHC (matches enfortumab vedotin's and sacituzumab govitecan's documented clinical skin toxicity, `TACSTD2` reaching IHC-High, the only High-level rows seen across all five targets run so far). One real, notable gap: **`F3` has no usable HPA IHC data** — exactly one row, `Tissue=N/A`/`Level=N/A`/`Reliability=Uncertain` (confirmed by direct inspection, not a script bug) — its normal-tissue assessment rests on bulk RNA alone, this module's "protein/IHC-first triage" principle genuinely not available for this one target. See each target's `question/`, `data_lock/`, `analysis_contracts/tgt_<x>.md`.
