...
2. **QC filtering, using the paper's own published thresholds** (Methods, Cell Discovery 2021): detected genes >= 500; mitochondrial UMI fraction <= 15% (13 canonical `MT-` protein-coding genes present in the gene index); per-sample (used as this dataset's "batch" unit) 3-SD outlier removal on both `log10(total UMI)` and detected-gene count. This run: 123,330 of 140,281 barcodes pass (the paper reports 111,292 of 140,281 after its own pipeline — not expected to match exactly, since the paper's batch definition and exact per-cell arithmetic are not independently reproducible from the Methods text alone; reported as a sanity comparison, not a target to hit).
