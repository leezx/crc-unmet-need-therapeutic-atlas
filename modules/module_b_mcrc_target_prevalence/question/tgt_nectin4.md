...
Same method as prior targets (`data_lock/tgt_nectin4.md`, `analysis_contracts/tgt_nectin4.md`): QC-filtered, EPCAM-based epithelial-proxy screen, **not malignancy-confirmed**, no CNV-based confirmation attempt for this target in this pass. `TE020` (3 treated patients) and `TE021` (3 treatment-naive patients, context evidence) are the real, QC-filtered result. `GSE225857` remains genuinely blocked on CNSA access; no row exists for it.
