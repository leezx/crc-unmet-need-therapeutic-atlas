...
**Attempted 2026-08-23** via `scripts/infercnv_lite_gse178318.py` — a chromosome-arm-level CNV score (gene-position reference from `DATA/1.Databases/HGNC_gene_id_mapping`, resolved via `config/external_sources.yaml`'s `module_b_gene_position_reference` `path_env_var`) against a tumor-site-immune reference population. **Result: underpowered, not confirmatory** — see `analysis_contracts/infercnv_lite_gse178318.md` for the full method and honest results. Epithelial-proxy cells show a real, modest median CNV-score elevation over the reference (directionally consistent with malignancy) but the two distributions overlap substantially, and only ~1% of cells individually clear a statistically-principled threshold — too few per sample for a reliable confirmed-malignant CEACAM5 prevalence number. This is a genuine limitation of a single-cell, no-cross-cell-smoothing arm-level method (coarser than the source publication's own gene-window InferCNV), not evidence that the epithelial-proxy population isn't malignant. **Still open**: a finer-grained, gene-window InferCNV reproduction (or a real InferCNV package run, if one becomes available in this environment) could have more power than this arm-level attempt. For `GSE225857`: CNSA access-terms review, then a real fetch, unchanged.
