...
Current source-only evidence objects: 11 (`evidence.tsv`) — 8 pre-pivot, dataset-provenance-level (`target_id = NA`), plus 3 target-specific rows (`EV009`-`EV011`, `tgt_ceacam5`, Module E). `target_evidence.tsv`: 3 rows (`TE001`-`TE003`), all `tgt_ceacam5` / Module E / `indication_id=mcrc_preop_chemotherapy_crlm` — `TE001`/`TE002` RNA (`UNCALIBRATED_PROXY`), `TE003` HPA IHC (`CALIBRATED_PROXY`). DepMap 26Q1 is represented only at release level; its CRC subset and dependency results remain unmaterialized, and per `DATA/registry/module_classification.tsv` it is `SUPPLEMENT_FROZEN` (activate only per-target, after shortlist, for a named mechanistic question).
