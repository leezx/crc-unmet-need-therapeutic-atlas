...
The first 8 `evidence.tsv` objects predate the pivot, are dataset-provenance-level (not target-specific), and keep `target_id = NA`. As of 2026-08-23, `evidence.tsv` gained 2 real target-specific rows (`EV009`/`EV010`) and `target_evidence.tsv` gained its first 2 real rows (`TE001`/`TE002`) — Module E's bulk-RNA normal-tissue screen for `tgt_ceacam5` (`CEACAM5`), one of the 23 targets in `ADC_TARGET_SEED_UNIVERSE.tsv` — see `modules/module_e_normal_tissue_risk/question|data_lock|analysis_contracts/tgt_ceacam5.md` and `scripts/extract_normal_tissue_rna.py`. Module B's paired attempt for the same target is recorded as `BLOCKED` (no processed scRNA data locally available, both source datasets still `CANDIDATE`), not force-filled — see `modules/module_b_mcrc_target_prevalence/data_lock/tgt_ceacam5.md`. See `schemas/target_seed.tsv` (Module A's `ADC_TARGET_SEED_UNIVERSE.tsv` input/admission contract) in the repository-root `schemas/` directory.
...
Current source-only evidence objects: 10 (`evidence.tsv`) — 8 pre-pivot, dataset-provenance-level (`target_id = NA`), plus 2 target-specific rows (`EV009`/`EV010`, `tgt_ceacam5`, Module E). `target_evidence.tsv`: 2 rows (`TE001`/`TE002`), both `tgt_ceacam5` / Module E / `evidence_directness=UNCALIBRATED_PROXY`. DepMap 26Q1 is represented only at release level; its CRC subset and dependency results remain unmaterialized, and per `DATA/registry/module_classification.tsv` it is `SUPPLEMENT_FROZEN` (activate only per-target, after shortlist, for a named mechanistic question).
