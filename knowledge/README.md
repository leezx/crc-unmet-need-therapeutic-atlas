...
The first 8 `evidence.tsv` objects predate the pivot, are dataset-provenance-level (not target-specific), and keep `target_id = NA`. As of 2026-08-23, `evidence.tsv` gained 4 real target-specific rows (`EV009`-`EV012`) and `target_evidence.tsv` gained its first 4 real rows (`TE001`-`TE004`), all `tgt_ceacam5` (`CEACAM5`, one of the 23 targets in `ADC_TARGET_SEED_UNIVERSE.tsv`) and all `indication_id=mcrc_preop_chemotherapy_crlm` — one dossier, not split across modules: Module E's normal-tissue screen (`TE001`/`TE002` bulk RNA, `TE003` HPA cell-type IHC) and Module B's `GSE178318` prevalence result (`TE004`, marker-score epithelial-cell CEACAM5 positivity — not malignancy calling — across the 6 matched primary/CRLM patients). All four rows are `evidence_directness=UNCALIBRATED_PROXY`. See `modules/module_e_normal_tissue_risk/question|data_lock|analysis_contracts/tgt_ceacam5.md`, `modules/module_b_mcrc_target_prevalence/question|data_lock|analysis_contracts/tgt_ceacam5.md`, `scripts/extract_normal_tissue_rna.py`, and `scripts/annotate_gse178318_cell_types.py`. `GSE225857` (this module's other `CORE_ACTIVE` dataset) remains genuinely blocked on CNSA access — not force-filled, no row for it. See `schemas/target_seed.tsv` (Module A's `ADC_TARGET_SEED_UNIVERSE.tsv` input/admission contract) in the repository-root `schemas/` directory.
...
Current source-only evidence objects: 12 (`evidence.tsv`) — 8 pre-pivot, dataset-provenance-level (`target_id = NA`), plus 4 target-specific rows (`EV009`-`EV012`, `tgt_ceacam5`). `target_evidence.tsv`: 4 rows (`TE001`-`TE004`), all `tgt_ceacam5` / `indication_id=mcrc_preop_chemotherapy_crlm` / `evidence_directness=UNCALIBRATED_PROXY` — `TE001`/`TE002` Module E RNA, `TE003` Module E HPA IHC, `TE004` Module B `GSE178318` prevalence (`measurement_layer` distinguishes RNA/IHC; none has earned `CALIBRATED_PROXY` or `DIRECT`). DepMap 26Q1 is represented only at release level; its CRC subset and dependency results remain unmaterialized, and per `DATA/registry/module_classification.tsv` it is `SUPPLEMENT_FROZEN` (activate only per-target, after shortlist, for a named mechanistic question).
