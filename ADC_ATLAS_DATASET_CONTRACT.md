...
- `derisking_tier`: `A_CLINICAL` (ADC in human trials/approved), `B_PRECLINICAL_ADC` (a real preclinical **ADC construct** exists — target-specific ADC in-vitro killing and/or in-vivo efficacy, or equivalent product-level ADC evidence), or `C_ANTIBODY_OR_BIOLOGY_ONLY` (target biology, or an antibody/epitope/internalization evidence exists but no ADC construct has actually been tested — this includes "antibody exists and internalizes" on its own, which is not ADC derisking).
- `repurposing_status`: `ACTIVE`, `FUTURE`, or `EXCLUDED`. **Default rule: only `A_CLINICAL` / `B_PRECLINICAL_ADC` targets default to `ACTIVE`; `C_ANTIBODY_OR_BIOLOGY_ONLY` defaults to `FUTURE`.** A target that merely appears in ADCdb, or merely has an internalizing antibody, is not, by itself, grounds to run it through Modules B–F — that would silently turn this into novel-ADC-feasibility discovery instead of indication repurposing for already-derisked ADC constructs.
...
| C — `REFRACTORY_PERSISTENCE` | X 到了真正的 2/3L refractory patient 身上还剩多少？Three distinct axes (`longitudinal_persistence`, `refractory_or_treated_presence`, `clinical_endpoint_context`) — see `modules/module_c_refractory_persistence/README.md` | GSE274551, GSE84267, GSE178318, GSE225857, GSE294385, target-specific clinical literature | Enrichment in a small paired cohort ≠ causal population-level claim (GSE84267 n=2, the only genuine paired pre/post design here). A single-timepoint refractory/treated biopsy (`refractory_or_treated_presence`: GSE274551, GSE178318, GSE225857, GSE294385) proves presence, not treatment-induced retention — it must never be labeled `longitudinal_persistence`. A first-line/pretreatment response-association cohort (`clinical_endpoint_context`: GSE196576, GSE235919, GSE5851) proves neither presence nor persistence. |
