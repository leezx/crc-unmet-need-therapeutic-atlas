# CRC Unmet-Need Therapeutic Atlas — project status (LEGACY, pre-2026-08-21 pivot)

**HISTORICAL — do not read this as current project state.** This is the project-status body as it stood through 2026-08-11 / PR #66, under the old Phase 1 (source-only registry) + Phase 2 (global fetal-state/plasticity malignant-cell-state discovery) direction. That direction was superseded on 2026-08-21 by the ADC Target Repurposing Atlas pivot (target-first, Modules A-F) — see `../../reports/PROJECT_STATUS.md` for current state, and `ARCHIVE_NOTE.md` in this directory for why. Moved here, not deleted, so this specific scorecard ("97/100", 19 candidates, 9 P0, Phase 2 gates) stops being read by an agent as the live status while remaining available as a provenance record.
